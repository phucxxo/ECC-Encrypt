/*
 * aes256.c — Triển khai AES-256 (FIPS 197)
 *
 * AES hoạt động trên "state" 4x4 bytes = 16 bytes = 128-bit block.
 * Mỗi round gồm 4 phép biến đổi:
 *   1. SubBytes:    Thay mỗi byte bằng S-box (phi tuyến)
 *   2. ShiftRows:   Dịch vòng từng hàng
 *   3. MixColumns:  Trộn từng cột trong GF(2^8)
 *   4. AddRoundKey: XOR với round key
 *
 * Round cuối bỏ MixColumns.
 * Key expansion: tạo 15 round keys từ key gốc 32 bytes.
 */

#include "aes256.h"
#include <string.h>

/* ========================================================
 * S-Box (Substitution Box) — 256 entries
 * Được tạo từ nghịch đảo trong GF(2^8) + affine transform
 * ======================================================== */
static const uint8_t SBOX[256] = {
    
};

/* Inverse S-Box */
static const uint8_t INV_SBOX[256] = {
    
};

/* Rcon — round constants cho key expansion */
static const uint32_t RCON[10] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
    0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000
};

/* ========================================================
 * Nhân trong GF(2^8) với đa thức tối giản x^8+x^4+x^3+x+1
 * xtime(a) = a<<1 (mod 0x11b)
 * ======================================================== */
static inline uint8_t xtime(uint8_t a) {
    return (a << 1) ^ ((a >> 7) ? 0x1b : 0x00);
}

static inline uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t result = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) result ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return result;
}

/* ========================================================
 * Key Expansion (Key Schedule)
 * AES-256: Nk=8 (8 words/key), Nr=14 rounds, 15 round keys × 4 words = 60 words
 * ======================================================== */

/* SubWord: áp dụng S-Box cho 4 bytes của một word */
static uint32_t sub_word(uint32_t w) {
    return ((uint32_t)SBOX[(w >> 24) & 0xff] << 24) |
           ((uint32_t)SBOX[(w >> 16) & 0xff] << 16) |
           ((uint32_t)SBOX[(w >>  8) & 0xff] <<  8) |
           ((uint32_t)SBOX[ w        & 0xff]);
}

/* RotWord: xoay vòng trái 8 bits */
static uint32_t rot_word(uint32_t w) {
    return (w << 8) | (w >> 24);
}

void aes256_init(aes256_ctx *ctx, const uint8_t key[AES_KEY_LEN]) {
    uint32_t *W = ctx->round_key;
    int Nk = 8;  /* AES-256: 8 words/key */

    /* Nạp key vào W[0..7] */
    for (int i = 0; i < Nk; i++) {
        W[i] = ((uint32_t)key[4*i]     << 24) |
               ((uint32_t)key[4*i + 1] << 16) |
               ((uint32_t)key[4*i + 2] <<  8) |
               ((uint32_t)key[4*i + 3]);
    }

    /* Mở rộng đến 60 words */
    for (int i = Nk; i < 4 * (AES_ROUNDS + 1); i++) {
        uint32_t temp = W[i - 1];
        if (i % Nk == 0) {
            temp = sub_word(rot_word(temp)) ^ RCON[i / Nk - 1];
        } else if (Nk > 6 && i % Nk == 4) {
            temp = sub_word(temp);
        }
        W[i] = W[i - Nk] ^ temp;
    }
}

/* ========================================================
 * Biến đổi state
 * State lưu dạng cột: state[col][row], col=0..3, row=0..3
 * Nạp từ bytes: state[0][0]=in[0], state[0][1]=in[1], ..., state[1][0]=in[4]...
 * ======================================================== */

/* AddRoundKey: XOR state với round key thứ round */
static void add_round_key(uint8_t state[4][4], const uint32_t *W, int round) {
    for (int c = 0; c < 4; c++) {
        uint32_t word = W[round * 4 + c];
        state[c][0] ^= (word >> 24) & 0xff;
        state[c][1] ^= (word >> 16) & 0xff;
        state[c][2] ^= (word >>  8) & 0xff;
        state[c][3] ^= (word      ) & 0xff;
    }
}

/* SubBytes: thay mỗi byte bằng SBOX */
static void sub_bytes(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[c][r] = SBOX[state[c][r]];
}

/* InvSubBytes */
static void inv_sub_bytes(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[c][r] = INV_SBOX[state[c][r]];
}

/* ShiftRows: dịch hàng r sang trái r vị trí */
static void shift_rows(uint8_t state[4][4]) {
    uint8_t tmp;
    /* Row 1: dịch trái 1 */
    tmp = state[0][1];
    state[0][1] = state[1][1]; state[1][1] = state[2][1];
    state[2][1] = state[3][1]; state[3][1] = tmp;
    /* Row 2: dịch trái 2 */
    tmp = state[0][2]; state[0][2] = state[2][2]; state[2][2] = tmp;
    tmp = state[1][2]; state[1][2] = state[3][2]; state[3][2] = tmp;
    /* Row 3: dịch trái 3 = dịch phải 1 */
    tmp = state[3][3];
    state[3][3] = state[2][3]; state[2][3] = state[1][3];
    state[1][3] = state[0][3]; state[0][3] = tmp;
}

/* InvShiftRows */
static void inv_shift_rows(uint8_t state[4][4]) {
    uint8_t tmp;
    /* Row 1: dịch phải 1 */
    tmp = state[3][1];
    state[3][1] = state[2][1]; state[2][1] = state[1][1];
    state[1][1] = state[0][1]; state[0][1] = tmp;
    /* Row 2: dịch phải 2 */
    tmp = state[0][2]; state[0][2] = state[2][2]; state[2][2] = tmp;
    tmp = state[1][2]; state[1][2] = state[3][2]; state[3][2] = tmp;
    /* Row 3: dịch phải 3 = dịch trái 1 */
    tmp = state[0][3];
    state[0][3] = state[1][3]; state[1][3] = state[2][3];
    state[2][3] = state[3][3]; state[3][3] = tmp;
}

/* MixColumns: nhân ma trận trong GF(2^8)
 * [ 2 3 1 1 ]   [ s0 ]
 * [ 1 2 3 1 ] × [ s1 ]
 * [ 1 1 2 3 ]   [ s2 ]
 * [ 3 1 1 2 ]   [ s3 ] */
static void mix_columns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++) {
        uint8_t s0 = state[c][0], s1 = state[c][1];
        uint8_t s2 = state[c][2], s3 = state[c][3];
        state[c][0] = gf_mul(0x02,s0) ^ gf_mul(0x03,s1) ^ s2 ^ s3;
        state[c][1] = s0 ^ gf_mul(0x02,s1) ^ gf_mul(0x03,s2) ^ s3;
        state[c][2] = s0 ^ s1 ^ gf_mul(0x02,s2) ^ gf_mul(0x03,s3);
        state[c][3] = gf_mul(0x03,s0) ^ s1 ^ s2 ^ gf_mul(0x02,s3);
    }
}

/* InvMixColumns: nhân ma trận nghịch đảo */
static void inv_mix_columns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++) {
        uint8_t s0 = state[c][0], s1 = state[c][1];
        uint8_t s2 = state[c][2], s3 = state[c][3];
        state[c][0] = gf_mul(0x0e,s0)^gf_mul(0x0b,s1)^gf_mul(0x0d,s2)^gf_mul(0x09,s3);
        state[c][1] = gf_mul(0x09,s0)^gf_mul(0x0e,s1)^gf_mul(0x0b,s2)^gf_mul(0x0d,s3);
        state[c][2] = gf_mul(0x0d,s0)^gf_mul(0x09,s1)^gf_mul(0x0e,s2)^gf_mul(0x0b,s3);
        state[c][3] = gf_mul(0x0b,s0)^gf_mul(0x0d,s1)^gf_mul(0x09,s2)^gf_mul(0x0e,s3);
    }
}

/* ========================================================
 * Encrypt/Decrypt
 * ======================================================== */

void aes256_encrypt_block(const aes256_ctx *ctx,
                          const uint8_t in[AES_BLOCK_LEN],
                          uint8_t out[AES_BLOCK_LEN]) {
    uint8_t state[4][4];

    /* Nạp input vào state (column-major) */
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[c][r] = in[c*4 + r];

    /* Initial AddRoundKey */
    add_round_key(state, ctx->round_key, 0);

    /* 13 rounds thông thường */
    for (int round = 1; round < AES_ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, ctx->round_key, round);
    }

    /* Round cuối: không có MixColumns */
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, ctx->round_key, AES_ROUNDS);

    /* Xuất output */
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            out[c*4 + r] = state[c][r];
}

void aes256_decrypt_block(const aes256_ctx *ctx,
                          const uint8_t in[AES_BLOCK_LEN],
                          uint8_t out[AES_BLOCK_LEN]) {
    uint8_t state[4][4];

    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[c][r] = in[c*4 + r];

    add_round_key(state, ctx->round_key, AES_ROUNDS);

    for (int round = AES_ROUNDS - 1; round >= 1; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, ctx->round_key, round);
        inv_mix_columns(state);
    }

    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, ctx->round_key, 0);

    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            out[c*4 + r] = state[c][r];
}
