...
static void mix_columns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++) {
        ...
        uint8_t s2 = state[c][2], s3 = state[c][3];
        state[c][0] = gf_mul(0x02, s0) ^ gf_mul(0x03, s1) ^ s2             ^ s3;
        state[c][1] = s0             ^ gf_mul(0x02, s1) ^ gf_mul(0x03, s2) ^ s3;
        state[c][2] = s0             ^ s1             ^ gf_mul(0x02, s2) ^ gf_mul(0x03, s3);
        state[c][3] = gf_mul(0x03, s0) ^ s1             ^ s2             ^ gf_mul(0x02, s3);
    }
}
...
static void inv_mix_columns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; c++) {
        uint8_t s0 = state[c][0], s1 = state[c][1];
        uint8_t s2 = state[c][2], s3 = state[c][3];
        state[c][0] = gf_mul(0x0e,s0) ^ gf_mul(0x0b,s1) ^ gf_mul(0x0d,s2) ^ gf_mul(0x09,s3);
